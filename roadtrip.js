...
state.grain=clamp(Number(qs.get('grain')??3),0,10);state.format=['wide','portrait','cinema'].includes(qs.get('format'))?qs.get('format'):'wide';
state.quality=['balanced','high','ultra'].includes(qs.get('quality'))?qs.get('quality'):'high';state.time=Math.max(0,Number(qs.get('t'))||0);
if(qs.get('paused')==='1')state.playing=false;
function status(s){$('status').textContent=s;}
function sceneUrl(){const u=new URL(location.href);u.search='';u.searchParams.set('seed',canonical());u.searchParams.set('format',state.format);u.searchParams.set('grain',state.grain);u.searchParams.set('quality',state.quality);return u;}
function sync(){
 $('seed').value=canonical();$('speed').value=state.speed;$('speedLabel').textContent=Math.round(state.speed*3.6)+' km/h';$('grain').value=state.grain;$('grainLabel').textContent=state.grain===0?'Off':state.grain<5?'Subtle':'Textured';$('quality').value=state.quality;
 $('shotLabel').textContent=SHOTS[state.shot][0];$('lensLabel').textContent=SHOTS[state.shot][1]+' MM';
...
function positionAt(z,offset=1.65){return new T.Vector3(world.roadX(z)+offset,world.roadY(z)+.13,z-world.origin);}
function pose(kind,z,time){
 const choices={C:[-19,4.8,6.0],L:[-11,-1.5,2.6],S:[-1.7,-9,4.2],A:[-30,-25,30],F:[16,5.8,4.8]};const [dz,dx,h]=choices[kind]||choices.C;
...
function draw(time){
 ...
 camera.position.copy(cam);const target=positionAt(z+(state.shot==='A'?7:2.4));target.y+=.9;camera.lookAt(target);camera.rotation.z+=Math.sin(time*.12)*.002;
 ...
 key.target.position.set(Math.round(p.x/snap)*snap,p.y,Math.round(p.z/snap)*snap);key.position.copy(key.target.position).addScaledVector(sun,180);key.target.updateMatrixWorld();
...
async function build(){
 try{
  ...
  sync();resize();draw(state.time);state.ready=true;$('loading').hidden=true;
  $('scene').dataset.ready='true';$('scene').dataset.msaa=post.target.samples;$('scene').dataset.materialAtlas=materials.atlasSize.join('x');
...
async function exportVideo(options={}){
 ...
 try{
  ...
  const blob=new Blob([bytes],{type:'video/mp4'}),name=`grand-tour-${canonical()}-${seconds}s.mp4`;captureUrl=URL.createObjectURL(blob);
  ...
  $('exportDialog').dataset.frames=chunks;$('exportDialog').dataset.bytes=blob.size;$('exportDialog').dataset.start=start;status('Film rendered. Use Download MP4 or Share video.');
