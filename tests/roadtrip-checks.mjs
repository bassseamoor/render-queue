...
for(const seed of ['9A71C3','000000','FFFFFF','14FA30','777777']){
 ...
 const a=v.buildTerrain(0),b=v.buildTerrain(1),pa=a.getAttribute('position'),pb=b.getAttribute('position'),na=a.getAttribute('normal'),nb=b.getAttribute('normal');
 for(let i=0;i<41;i++){
  const j=pa.count-41+i;maxPositionSeam=Math.max(maxPositionSeam,Math.abs(pa.getX(j)-pb.getX(i)),Math.abs(pa.getY(j)-pb.getY(i)));
