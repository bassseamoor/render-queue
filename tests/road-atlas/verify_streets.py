...
REPORT={'version':'2.4.0','execution':'Local exact seven-resource async bootstrap; headless Chromium, independent Shapely geometry','sourceSHA256':{n:hashlib.sha256((ROOT/n).read_bytes()).hexdigest() for n in NAMES},'checks':[],'seeds':[],'errors':[],'warnings':[]}
...
with sync_playwright() as pw:
 ...
 for si,seed in enumerate(SEEDS):
  ...
  check(seed+' shipped street module executed',page.evaluate("world.streetKit?.version==='2.4.0'"),page.locator('#err').text_content())
