// Parallax Engine — SeedConsole adapter (from parallax-engine.html;
// modularized: studio state comes from window.__parallaxHooks, exposed by ../app.js).

/* ============ Seed Console adapter: Parallax Engine ============ */
(function () {
  'use strict';
  var H = window.__parallaxHooks; // exposed by ../app.js (modularized; was shared script scope)
  if (!H) { console.error('[parallax-engine] SeedConsole adapter: studio hooks missing'); return; }
  ...
  window.SeedConsoleAdapter = {
    ...
    fileBase: function (seedStr) { return 'parallax-' + String(seedStr).replace(/[^\w\-]+/g, '_'); }
  };
