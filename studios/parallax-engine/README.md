# Parallax Engine — module source map

Modularized from the single-file `parallax-engine.html` (script blocks 0–4,
in order). The HTML file is now a thin loader: identical markup/CSS, one
`<script type="module" src="studios/parallax-engine/main.js">`.
...
| `ui/adapter.js` | block 2, re-hooked | Parallax Engine adapter: `window.SeedConsoleAdapter`. Reads studio state through `window.__parallaxHooks` (random/remix/params/getSeed/setSeed). |
...
- `window.__parallaxHooks` — **new, additive**: exposes the adapter's needs
...
- `app.js` keeps the studio core as one module (same as the Molten and Canal
  slices); shader data was the clean separable seam. A deeper split of
  `app.js` (realm builders / city systems / render loop) is possible as a follow-up.
